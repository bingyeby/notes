一次凉凉的前端面试 https://zhuanlan.zhihu.com/p/51607206

一年半经验，百度、有赞、阿里前端面试总结 https://juejin.im/post/5befeb5051882511a8527dbe

刷《一年半经验，百度、有赞、阿里面试总结》·手记 https://godbmw.com/passages/2018-11-28-try-web-interview/
...
### new操作符具体干了什么呢?
	...
	于是，这也要求我们在对prototype重新赋值的时候，重新指定constructor属性。F.prototype={
		constructor:F
	};
...
//[ig'zek] [ 'es',  index: 7,  input: 'test, Tes, tst, tset, Test, Tesyt, sTes' ]

console.log(str.search(reg));// 1
console.log(str.match(reg));// [ 'es', 'es', 'es', 'es', 'es' ]

console.log(str.indexOf("es"));//1   //不接受正则作为参数
console.log(str.lastIndexOf("es"))//38

'aaaa'.replace(/\w/g, function(value) { return value.toUpperCase(); }); 

'12345432'.replace(/(?=(?!\b)(\d{3})+$)/g, ',');  
// "12,345,432"  (?=) :前向声明  (?!) :反前向声明
```
...
[exports与module.exports还有import](https://blog.csdn.net/wopelo/article/details/78890566) 
#### CommonJS模块规范
...
2. 在引入文件中，如果需要使用export.js中的函数，用户必须先知道它的名字，如果用户不想翻文档或者怎么滴，这样子就不太方便，所以就有了export default
